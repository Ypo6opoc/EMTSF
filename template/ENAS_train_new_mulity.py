...
if __name__ == "__main__":
    ...
    with open(args.load_genotypes, "r") as f:
        genotypes = yaml.load(f, Loader=yaml.FullLoader)
        if amae.mean() != 0:
            genotypes['fitness'] = float(amae.mean())
            # genotypes['records_mae'] = amae
            # genotypes['records_rmse'] = armse
            # genotypes['records_mape'] = amape
        else:
            genotypes['fitness'] = float(101)
            # genotypes['records_mae'] = amae
            # genotypes['records_rmse'] = armse
            # genotypes['records_mape'] = amape
    with open(args.load_genotypes, "w") as f:
        yaml.dump(genotypes, f)

    args.static_record = "/home/liangzixuan/ENAS_MTSF/MST_NAS/records/"
    if not os.path.exists(args.static_record):
        os.makedirs(args.static_record)
    args.static_record += genotypes_id + ".json"
    data = {'mae':amae,
            'rmse':armse,
            'mape':amape}
    with open(args.static_record, 'w') as file:
        json.dump(data, file)
"""
