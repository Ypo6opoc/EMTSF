...
from models.operations_new import OPS
    ...
    for dst in range(1, args.nb_nodes*3+1):
        ...
        best_links = sorted(
            ...
            key = lambda lk: -max(
                ...
                if 'V_None' not in link[dst][lk][-2] or j != link[dst][lk][-2].index('V_None') or 'TC_None' not in
                link[dst][lk][-2] or j != link[dst][lk][-2].index('TC_None')
            )
        )[:1] #! 截取的操作数量
        for blink in best_links:
            blink = link[dst][blink]
            if 'V_None' in blink[-2] or 'TC_None' in blink[-2]:
...
def genotypes(args, arch_paras, arch_topos):
    result = {'Genotype': []}
    for id in range(args.layers * args.GNN_depth_per_block):
        cell_result = cell_genotype(args, id, arch_paras[id], arch_topos[id])
        result['Genotype'].append(cell_result)
    return result

def TC_genotypes(args, arch_paras, arch_topos):
    result = {'Genotype': []}
    for id in range(args.layers):
        cell_result = cell_genotype(args, id, arch_paras[id], arch_topos[id])
        result['Genotype'].append(cell_result)
    return result
