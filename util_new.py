...
class DataLoaderS(object):
    ...
    def __init__(self, args, file_name, train, valid, device, horizon, window, normalize=2):
        self.P = window
        self.h = horizon
        # filename = r'data/h5data/' + dataset + '.h5'
        self.rawdat= (pd.read_hdf(file_name)).to_numpy()
        ...
        feature_list = [np.expand_dims(self.dat, axis=-1)]
        # args.add_time_in_day, args.add_day_in_week=True, True
        # args.steps_per_day = 12
        if args.add_time_in_day:
            # numerical time_in_day
            time_ind = [i % args.steps_per_day / args.steps_per_day for i in range(self.dat.shape[0])]
            time_ind = np.array(time_ind)
            time_in_day = np.tile(time_ind, [1, self.m, 1]).transpose((2, 1, 0))
            feature_list.append(time_in_day)
        if args.add_day_in_week:
            # numerical day_in_week
            day_in_week = [(i // args.steps_per_day) % 7 for i in range(self.dat.shape[0])]
            day_in_week = np.array(day_in_week)
            day_in_week = np.tile(day_in_week, [1, self.m, 1]).transpose((2, 1, 0))
            feature_list.append(day_in_week)
        self.raw_data = np.concatenate(feature_list, axis=-1)
        self.f = self.raw_data.shape[-1]
        ###

        self._split(int(train * self.n), int((train + valid) * self.n), self.n)

        self.static_feat = self.rawdat[:int(train * self.n)]
    ...
    def _batchify(self, idx_set, horizon):
        ...
        for i in range(n):
            ...
            start = end - self.P
            X[i, :, :, :] = torch.from_numpy(self.raw_data[start:end, :, :])
            Y[i, :] = torch.from_numpy(self.dat[idx_set[i], :])
        return [X, Y]
...
def load_dataset(dataset, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    for category in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(dataset, category + '.npz'))
